    ...
    def _serialize_event(self, event: RabbitMQOutboxEvent) -> bytes:
        match event.event_type:
            case "stock.reservation.failed":
                failed_payload = StockReservationFailedV1.model_validate(event.payload)
                envelope = StockReservationFailedEnvelopeV1(
            ...
            case "stock.reserved":
                succeeded_payload = StockReservedV1.model_validate(event.payload)
                envelope = StockReservedEnvelopeV1(
                    event_id=event.event_id,
                    event_type=event.event_type,
                    event_version=event.event_version,
                    occurred_at=event.occurred_at,
            ...
            case unknown_event_type:
                raise ValueError(
                    f"Unsupported stock reservation event type: {unknown_event_type}"
                )
