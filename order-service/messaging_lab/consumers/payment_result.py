...
async def retry_or_send_to_dlq(
    ...
    try:     
        ...
        logger.exception(
            "Failed to publish payment result message to retry queue: message_id=%s",
            message.message_id,
        )
        await message.reject(requeue=False)
        return
...
async def handler_payment_result(
    ...
    try:
        ...
        logger.exception(
            "Unexpected payment_result message parsing error: message_id=%s",
            message.message_id,
        )
        await message.reject(requeue=False)
        return
    ...
    try:
    ...
    except PermanentPaymentResultError as exc:
        logger.error(
            "Permanent payment result error: message_id=%s error=%s",
            message.message_id,
            exc,
        )
        await message.reject(requeue=False)
        ...
        logger.exception(
            "Temporary payment result error: message_id=%s",
            message.message_id,
        )
        ...
        logger.exception(
            "Unexpected payment result error: message_id=%s",
            message.message_id,
        )
        await retry_or_send_to_dlq(
            message=message,
